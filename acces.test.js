...
test("une création de compte est contrôlée avant tout envoi", () => {
  ...
  assert.match(Acces.verifierDemande({ ...ok, mail: "marie@gmail.com" }), /professionnelle/);
  assert.match(Acces.verifierDemande({ ...ok, motDePasse: "court" }), /8 caractères/);
  assert.strictEqual(Acces.domaineAutorise("Marie.Durand@GROUPE-IDEA.COM"), true);
});
...
test("les règles et le module parlent des mêmes rôles et des mêmes domaines", () => {
  ...
  const roles = ((R.match(/role in \[([^\]]*)\]/) || [])[1] || "").split(",").map(x => x.trim());
  assert.ok(roles.includes("''"), "« en attente » (rôle vide) doit rester possible");
  assert.deepStrictEqual(roles.filter(x => x !== "''").map(x => x.replace(/'/g, "")).sort(),
    Object.keys(Acces.ROLES).sort());
  Acces.DOMAINES.forEach(d =>
    assert.ok(R.includes(d.replace(/\./g, "[.]")), "domaine absent des règles : " + d));
...
test("une adresse hors domaine est refusée avant de créer quoi que ce soit", async () => {
  await appareilComptes();
  A.run("basculerCreation(true)");
  A.saisir("compteNom", "Pirate").saisir("compteMail", "pirate@gmail.com").saisir("compteMdp", "unbonmotdepasse");
  assert.strictEqual(await A.run("creationCompte()"), false);
  assert.deepStrictEqual(A.comptesCrees(), []);
  assert.match(A.texte("porteMessage"), /professionnelle/);
});
...
test("le panneau liste les demandes et tous les noms existants, avec leurs données", async () => {
  ...
  assert.match(liste, new RegExp(`id="accesNom_${uidMarie}"[^>]*>[\\s\\S]*?<option value="Marie" selected>`));
