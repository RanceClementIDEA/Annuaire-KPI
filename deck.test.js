...
test("mode : un lien de visuel sans empreinte demande d'abord un relevé", () => {
  ...
  assert.ok(A.html("deckList").includes("visuel indisponible"),
...
test("liens : un visuel sans empreinte est signalé dans le bilan", () => {
  ...
  assert.match(A.texte("deckWarning"), /sans visuel disponible/);
});
...
test("liens : un lien de PAGE est signalé — il afficherait tout le rapport", () => {
  ...
  assert.match(A.texte("deckWarning"), /lien de page entière/);
});
...
test("liens : un visuel dix fois plus large que haut est signalé comme suspect", () => {
  ...
  assert.match(A.texte("deckWarning"), /très allongé/);
});
...
test("liens : le bilan cumule les différents soucis", () => {
  ...
  assert.match(bilan, /1 lien de page entière/);
  assert.match(bilan, /1 visuel très allongé/);
...
test("empreintes : relever un KPI ne couvre pas son voisin de la même page", () => {
  ...
  A.basculerSelection("kpi_taux_service_hebdomadaire");
  A.run("renderDeckLignes()");
  assert.equal((A.html("deckList").match(/visuel indisponible/g) || []).length, 2);
});
...
test("empreintes : le bilan annonce des KPI à insérer, pas des pages", async () => {
  ...
  A.run("renderDeckLignes()");
  const bilan = A.texte("deckWarning");
  assert.match(bilan, /2 diapositive\(s\) sans visuel disponible/);
  /* Court : le bilan dit QUOI faire, pas pourquoi. Le pourquoi est dans
     « ❓ Guide », et la fenêtre doit rester lisible d'un coup d'œil. */
  assert.ok(bilan.length < 200, "bilan de " + bilan.length + " caractères : " + bilan);
});
...
test("empreintes : un relevé .json couvre le KPI qu'il décrit, et lui seul", async () => {
  ...
  assert.equal((html.match(/visuel indisponible/g) || []).length, 1);
});
...
test("signet : sur un AUTRE signet, la ligne dit que le lien a été repartagé", async () => {
  ...
  A.run("renderDeckLignes()");
  const html = A.html("deckList");
  assert.ok(html.includes("lien à mettre à jour"), html);
  assert.ok(!html.includes("⚡ visuel"), "surtout pas de « ⚡ visuel » rassurant");
});

test("signet : un visuel sans empreinte dit simplement « indisponible »", () => {
  preparerDeck();
  relier("kpi_volumetrie_hebdomadaire", SIGNET_KPI);
  A.basculerSelection("kpi_volumetrie_hebdomadaire");
  A.run("renderDeckLignes()");
  const html = A.html("deckList");
  assert.ok(html.includes("visuel indisponible"));
  assert.ok(!html.includes("à mettre à jour"), "aucun lien n'a changé ici");
});

test("signet : le bilan reste court — c'est la LIGNE qui nomme le repartage", async () => {
  preparerDeck();
  relier("kpi_volumetrie_hebdomadaire", SIGNET_VOISIN);
  A.basculerSelection("kpi_volumetrie_hebdomadaire");
  await A.run("releverEmpreintesDepuis")(pptxAvecComplement(SIGNET_KPI));
  A.run("renderDeckLignes()");
  assert.match(A.html("deckList"), /à mettre à jour/, "la ligne le dit");
  assert.ok(A.texte("deckWarning").length < 200,
    "le bilan ne répète pas l'explication : " + A.texte("deckWarning"));
});
...
/* ─── Dire POURQUOI une insertion a été écartée ─────────────
   Vérifié sur un fichier réel : le complément, ne pouvant résoudre le
   signet d'un lien, était retombé sur la page d'accueil du rapport et
   avait mémorisé CETTE page. L'empreinte est justement refusée — mais
   le refus doit s'expliquer, sinon on cherche longtemps. */

/** Un complément dont l'état appartient à une AUTRE page que le lien. */
function pptxAutrePage(lien) {
  const props = [
    `<we:property name="reportUrl" value="&quot;${lien.replace(/&/g, "&amp;")}&quot;"/>`,
    `<we:property name="artifactName" value="&quot;Histo&quot;"/>`,
    `<we:property name="pageName" value="&quot;pageDAccueil&quot;"/>`,
    `<we:property name="bookmark" value="&quot;H4sIEtat&quot;"/>`
  ].join("");
  return A.run(`ZipMini.ecrireZip([
    { nom: "[Content_Types].xml", donnees: '<?xml version="1.0"?><Types></Types>' },
    { nom: "ppt/webextensions/webextension1.xml",
...
test("relevé : un signet mort est nommé, pas silencieusement ignoré", async () => {
  preparerDeck();
  relier("kpi_volumetrie_hebdomadaire", SIGNET_KPI);
  const bilan = await A.run("releverEmpreintesDepuis")(pptxAutrePage(SIGNET_KPI));
  assert.equal(bilan.total, 0, "rien n'est relevé");
  assert.equal(bilan.motifs.autrepage, 1, "et la cause est identifiée");
  assert.match(A.run("phraseRefus")(bilan.motifs), /signet.*n'existe plus/,
    "le message doit dire que le signet est mort");
});

test("relevé : une diapositive fabriquée est distinguée d'un signet mort", () => {
  const sans = { reportUrl: "&quot;" + SIGNET_KPI.replace(/&/g, "&amp;") + "&quot;" };
  assert.equal(A.run("Empreintes.raisonRefus")(sans), "fabrique");
  assert.equal(A.run("Empreintes.raisonRefus")({ reportUrl: "&quot;https://exemple/&quot;" }), "lien");
});

test("relevé : une insertion valable n'a aucun motif de refus", () => {
  const props = {
    reportUrl: "&quot;" + SIGNET_KPI.replace(/&/g, "&amp;") + "&quot;",
    artifactName: "&quot;Histo&quot;",
    bookmark: "&quot;H4sIEtat&quot;"
  };
...
/* ─── Le même KPI sur plusieurs zones ──────────────────────
   ...
   cocher une zone AJOUTE une diapositive, elle n'en remplace aucune. */

test("zones : par défaut, une seule diapositive par KPI", () => {
  preparerDeck();
  relier("kpi_volumetrie_hebdomadaire", SIGNET_KPI);
  A.basculerSelection("kpi_volumetrie_hebdomadaire");
  assert.equal(A.run("diaposCourantes()").diapos.length, 1);
});

test("zones : cocher une seconde zone ajoute une diapositive", () => {
  preparerDeck({ manualEntries: [{ id: "kpi_multi", manual: true, title: "Volumétrie",
    freq: "Hebdomadaire", ritual: "COPIL", _mtime: 1, _by: "c",
    logistiport: SIGNET_KPI, armement: SIGNET_VOISIN }] });
  A.basculerSelection("kpi_multi");
  assert.equal(A.run("diaposCourantes()").diapos.length, 1, "une seule au départ");
  A.run("basculerZone")("kpi_multi", "logistiport");
  A.run("basculerZone")("kpi_multi", "armement");
  const diapos = A.run("diaposCourantes()").diapos;
  assert.equal(diapos.length, 2, "deux diapositives, deux zones");
  assert.deepEqual(diapos.map(d => d.site), ["logistiport", "armement"]);
  assert.deepEqual(diapos.map(d => d.lien), [SIGNET_KPI, SIGNET_VOISIN],
    "chaque diapositive porte le lien de SA zone");
});

test("zones : décocher retire la diapositive correspondante", () => {
  preparerDeck({ manualEntries: [{ id: "kpi_multi", manual: true, title: "Volumétrie",
    freq: "Hebdomadaire", ritual: "COPIL", _mtime: 1, _by: "c",
    logistiport: SIGNET_KPI, armement: SIGNET_VOISIN }] });
  A.basculerSelection("kpi_multi");
...
test("zones : une zone sans lien n'est jamais proposée", () => {
  preparerDeck({ manualEntries: [{ id: "kpi_multi", manual: true, title: "Volumétrie",
    freq: "Hebdomadaire", ritual: "COPIL", _mtime: 1, _by: "c", logistiport: SIGNET_KPI }] });
  A.basculerSelection("kpi_multi");
  assert.deepEqual(A.run("zonesDisponibles")("kpi_multi"), ["logistiport"]);
});

test("zones : « toutes les zones » couvre la sélection entière", () => {
  preparerDeck({ manualEntries: [{ id: "kpi_multi", manual: true, title: "Volumétrie",
    freq: "Hebdomadaire", ritual: "COPIL", _mtime: 1, _by: "c",
    logistiport: SIGNET_KPI, armement: SIGNET_VOISIN }] });
  A.basculerSelection("kpi_multi");
  A.run("toutesLesZones")();
  assert.equal(A.run("diaposCourantes()").diapos.length, 2);
  A.run("uneSeuleZone")();
  assert.equal(A.run("diaposCourantes()").diapos.length, 1, "et l'on peut revenir en arrière");
});

test("zones : le commentaire appartient à la ligne, pas au KPI", () => {
  preparerDeck({ manualEntries: [{ id: "kpi_multi", manual: true, title: "Volumétrie",
    freq: "Hebdomadaire", ritual: "COPIL", _mtime: 1, _by: "c",
    logistiport: SIGNET_KPI, armement: SIGNET_VOISIN }] });
  A.basculerSelection("kpi_multi");
  A.run("toutesLesZones")();
  A.run("noterCommentaire")("kpi_multi", "vu au COPIL", "armement");
  const d = A.run("diaposCourantes()").diapos;
  const com = A.run(`diaposCourantes().diapos.map(d => d.commentaire || commentaireDe(d))`);
  assert.equal(com[d.findIndex(x => x.site === "armement")], "vu au COPIL");
  assert.equal(com[d.findIndex(x => x.site === "logistiport")], "",
    "la zone voisine garde son propre commentaire");
});

test("zones : une sélection d'avant, sans zones, ne change pas de comportement", () => {
  const preset = A.run(`Selection.normaliserPreset({ name: "COPIL",
    items: [{ kpiId: "a" }, { kpiId: "a" }, { kpiId: "b" }] })`);
  assert.equal(preset.items.length, 2, "les doublons sans zone restent fusionnés");
});

test("zones : le même KPI sur deux zones fait bien deux lignes distinctes", () => {
  const preset = A.run(`Selection.normaliserPreset({ name: "COPIL",
    items: [{ kpiId: "a", site: "logistiport" }, { kpiId: "a", site: "global" },
            { kpiId: "a", site: "global" }] })`);
  assert.equal(preset.items.length, 2, "deux zones, deux lignes ; le doublon exact tombe");
});
...
/* Le support doit EMPORTER les empreintes déduites.
   La fenêtre annonçait « ✨ déduit », mais la fabrique ne recevait que les
   empreintes relevées : chaque ligne déduite sortait muette, et affichait
   « L'objet visuel ajouté ici n'existe plus ». */

test("génération : une ligne déduite emporte bien son état dans le support", async () => {
  preparerAxes();
  await A.run("releverEmpreintesDepuis")(await pptxAvecEtat(R1, "LOG"));
  await A.run("releverEmpreintesDepuis")(await pptxAvecEtat(R3, "QUOTI"));
  const bilan = await A.run("engendrerEmpreintes")();
  if (!bilan.engendrees) return;   // sans axe utilisable, rien à vérifier

  const utilisables = A.run("empreintesUtilisables()");
  assert.ok(utilisables.length > A.run("empreintes.length"),
    "les déduites s'ajoutent aux relevées");
  const resolu = A.run(`Empreintes.resoudre(empreintesUtilisables(), ${JSON.stringify(R2)})`);
  assert.ok(resolu && resolu.proprietes && resolu.proprietes.bookmark,
    "la fabrique trouve un état pour la ligne déduite");
});

test("génération : une empreinte relevée prime sur la déduite dans le support", async () => {
  preparerAxes();
  await A.run("releverEmpreintesDepuis")(await pptxAvecEtat(R1, "LOG"));
  await A.run("releverEmpreintesDepuis")(await pptxAvecEtat(R2, "ARM"));
  await A.run("releverEmpreintesDepuis")(await pptxAvecEtat(R3, "QUOTI"));
  await A.run("engendrerEmpreintes")();
  const liste = A.run("empreintesUtilisables()");
  const premier = liste.findIndex(e => e.id === A.run(`Empreintes.cleVisuel(${JSON.stringify(R2)})`));
  assert.ok(premier < A.run("empreintes.length"),
    "la relevée est trouvée avant toute déduite");
});
